...
| **Vault** | NIP-44 + NIP-17/NIP-59 (gift-wrapped); legacy NIP-04 read-only | End-to-end encrypted direct messages; replies (NIP-10 `e` tag), delivered/read receipts, **emoji reactions**, `/pay` UPI intent detection | ✅ Send + receive wired (desktop + Android), offline chat history persisted. New DMs are NIP-44-encrypted and gift-wrapped (no sender/content metadata leaks to relays); a peer's older NIP-04 DMs still decrypt. Reactions ride the same sealed channel as a *control envelope*, **not** a public NIP-25 Kind-7 event — a kind-7 reaction tags both the event and its author in the clear, so reacting to a gift-wrapped DM would tell every relay these two keys are talking |
