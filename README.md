...
| **App updates** | GitHub Releases + a version rule shared by both frontends | Comrade is installed by sideload, so nothing otherwise tells anyone that a release — including a security fix — shipped. A once-a-day check of the project's own `releases/latest` raises a low-priority notification ("Comrade 0.0.9 is available"), and a Settings card shows the running version, the release notes, "Get the update", and "Skip this version" (which silences that version, not the next one). **The app never downloads or installs an APK**: the release page opens in the browser — already the install source these users granted — so Comrade holds no `REQUEST_INSTALL_PACKAGES`, and Android's own same-signature rule still gates what can land as an update. The daily check is a real disclosure (your IP address, to GitHub) and the card says so in as many words; it is a switch, and turning it off leaves a manual "Check now" | ✅ Wired (Compose + Flutter; the rule and the fetch are one shared native implementation, unit-tested on the host JVM). Not on desktop: no desktop artifact is published to update *to* |
