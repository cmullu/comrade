 ...
 * (`ChatEventRouter.route`). A second drainer in Dart would silently steal
 * call offers from the code that makes the phone ring.
 *
 * The single consumer is `EventPump`, not this service. That distinction is
 * not pedantry: the service is gated on a user preference, so while it owned
 * the loop, turning the preference off stopped delivery even with the app
 * open. `EventPump` is held by whoever needs it — this service while it runs,
