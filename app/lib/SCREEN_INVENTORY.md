...
| D26a | **Light-mode accents** | — | `--accent` (`#6366f1`, `#f59e0b`, `#38bdf8`, `#fb7185`) | **Darker steps of the same hues** | `styles.css` was dark-only when this was written, so every value in it was tuned against near-black, and there `--accent` (now `--primary`, since the shadcn adaptation renamed the tokens — `docs/DESIGN_SYSTEM.md`) is a *fill* carrying dark `--primary-foreground` text — never text itself. Desktop has had a light appearance since 2026-08-17 and reached the same conclusion independently: `body.theme-light` restates every colour token rather than reusing the dark ones. The first port reused it as light-mode `primary`, which `SectionCard` renders titles in: Travel measured **2.1:1** on white against AA's 4.5:1, and white-on-amber in a `FilledButton` was 2.15:1. Same for `ComradeSurfaces.light`'s good/warn/bad, which the sidebar pills use as *label* colours. `theme_test.dart` now asserts 4.5:1 across every text role × every background × all four skins × both brightnesses, so a palette edit cannot quietly reintroduce it. |
